...
class ActuControllerTest extends WebTestCase
{
  ...
  // Tests additionnels pour 100% de couverture

  public function testIndexWithNoResultsSearchAndCategory(): void
  {
    $author = $this->createAuthor('John Doe');
    $category = $this->createCategory('Tech', 'tech');
    $this->createPost('Article PHP', 'Contenu', $author, true, [$category]);

    $this->client->request('GET', '/actu?search=MotInexistant&category=Tech');

    $this->assertResponseIsSuccessful();
    $this->assertSelectorExists('.alert-info');
    $this->assertSelectorTextContains('.alert-info', 'Aucun article trouvé pour "MotInexistant" dans la catégorie "Tech"');
  }

  public function testIndexWithNoPostsAtAll(): void
  {
    $this->client->request('GET', '/actu');

    $this->assertResponseIsSuccessful();
    $this->assertSelectorExists('.alert-info');
    $this->assertSelectorTextContains('.alert-info', 'Aucun article publié pour le moment');
  }

  public function testIndexSearchWithOneResult(): void
  {
    $author = $this->createAuthor('John Doe');
    $this->createPost('Article Unique', 'Contenu unique', $author, true);

    $this->client->request('GET', '/actu?search=Unique');

    $this->assertResponseIsSuccessful();
    $this->assertSelectorExists('.alert-success');
    $this->assertSelectorTextContains('.alert-success', '1 article trouvé');
  }

  public function testIndexSearchWithMultipleResults(): void
  {
    $author = $this->createAuthor('John Doe');
    $this->createPost('Article Symfony 1', 'Contenu', $author, true);
    $this->createPost('Article Symfony 2', 'Contenu', $author, true);

    $this->client->request('GET', '/actu?search=Symfony');

    $this->assertResponseIsSuccessful();
    $this->assertSelectorExists('.alert-success');
    $this->assertSelectorTextContains('.alert-success', '2 articles trouvés');
  }

  public function testCreatePageShowsPostsCount(): void
  {
    $author = $this->createAuthor('John Doe');
    $this->createPost('Article 1', 'Contenu', $author, true);
    $this->createPost('Article 2', 'Contenu', $author, true);
  ...
  public function testEditPostWithInvalidData(): void
  {
    $author = $this->createAuthor('John Doe');
    $post = $this->createPost('Titre valide', 'Contenu', $author, true);
    ...
    // Le formulaire avec erreurs retourne un code 422 ou reste sur la page
    $this->assertResponseStatusCodeSame(422); // Unprocessable Content
    $this->assertSelectorExists('.alert-warning');
  }
  ...
  public function testIndexWithEmptySearch(): void
  {
    ...
    // Recherche avec chaîne vide ne doit pas filtrer
    $this->client->request('GET', '/actu?search=');
  ...
  public function testIndexWithEmptyCategory(): void
  {
    ...
    // Catégorie vide ne doit pas filtrer
    $this->client->request('GET', '/actu?category=');

    $this->assertResponseIsSuccessful();
  }

  public function testCreatePostDisplaysPostCount(): void
  {
    $author = $this->createAuthor('John Doe');
    $this->createPost('Article 1', 'Contenu', $author, true);
    $this->createPost('Article 2', 'Contenu', $author, false);
    $this->createPost('Article 3', 'Contenu', $author, true);

    $this->client->request('GET', '/actu/create');

    $this->assertResponseIsSuccessful();
    // Vérifie que les 3 posts (publiés et brouillons) sont comptés
